...
function DataBroker:OnInitialize()
    ...
    local BrokerPanel = LibStub('LibWindow-1.1'):Embed(CreateFrame('Button', nil, UIParent)) do
		local height = ElvUI[1].db.infobar.height or 22 --by eui.cc
        BrokerPanel:SetSize(160, height) --by eui.cc
		...
		BrokerPanel:SetTemplate("Transparent") --by eui.cc
    ...
    local BrokerIcon = BrokerPanel:CreateTexture(nil, 'ARTWORK') do
		local height = ElvUI[1].db.infobar.height or 22 --by eui.cc
        BrokerIcon:SetSize(height-2, height-2) --by eui.cc
