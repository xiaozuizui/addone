...
function GSE.GUICreateSequencePanels(frame, container, key)
  ...
  local SpecID = GSELibrary[classid][sequencename].SpecID
  if not SpecID then
	SpecID = GSE.GetCurrentSpecID()
  end
  SpecID = select(2, GetSpecializationInfoByID(tonumber(SpecID)))
  local label = AceGUI:Create("Label")
  label:SetFontObject(fontlarge)
  selpanel:AddChild(label)

  local hlabel = AceGUI:Create("Label")
  hlabel:SetText(SpecID and (SpecID.. '|'..sequencename) or sequencename)
