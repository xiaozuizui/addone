...
function UF:Update_PlayerFrame(frame, db)
	...
	if E.myclass == 'DEATHKNIGHT' then
		for i = 1, frame.MAX_CLASS_BAR do
			if db.classbar.dkText then
				frame.Runes[i].Text:SetPoint("CENTER", frame.Runes[i], "CENTER", db.classbar.dkTextXoffset, db.classbar.dkTextYoffset)
			else
				frame.Runes[i].Text:ClearAllPoints()
			end
		end
	end
