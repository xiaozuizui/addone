...
function UF:UpdateClickSet()
	...
	local spec = 'spec1'
	if self.ClickSet.specswap == true and MySpec == 2 then
		spec = 'spec2'
	elseif self.ClickSet.specswap == true and MySpec == 3 then
		spec = 'spec3'
	elseif self.ClickSet.specswap == true and MySpec == 4 then
		spec = 'spec4'
	end
	...
	for key, value in pairs(E.db["clickset"][spec]) do
		...
		elseif value ~= "NONE" and key ~= "enable" and type(value) == "string" then
			if sub(value, 1, 2) ~= "#*" then
				...
				self:SetAttribute(gsub(key_tmp,"type",'spell'), value)
			else
				self:SetAttribute(key_tmp, 'macro')
				self:SetAttribute(gsub(key_tmp,"type","macrotext"), sub(value, 3))
			end
		end
	end
end
