...
function mod:NAME_PLATE_UNIT_ADDED(_, unit, frame)
	...
	ClassNameplateManaBarFrame:Kill() --by 杀掉姓名版玩家框体的暴雪能量条
	ClassNameplateManaBarFrame.Show = E.noop
end
...
function mod:UpdateAllFrame(frame)
	if(frame == self.PlayerFrame__) then
		ClassNameplateManaBarFrame:Kill() --by 杀掉姓名版玩家框体的暴雪能量条
		ClassNameplateManaBarFrame.Show = E.noop
		return
	end
...
function mod:UpdateCVars()
	...
	--讓堆疊血條的間距小一點
	E:LockCVar("nameplateOverlapH",  self.db.nameplateOverlapH or 0.3) --default is 0.8
	E:LockCVar("nameplateOverlapV",  self.db.nameplateOverlapV or 0.7) --default is 1.1
	E:LockCVar("nameplateLargeTopInset", .08)
	E:LockCVar("nameplateLargeBottomInset", .1)
	E:LockCVar("nameplateOccludedAlphaMult", 0.2)--障礙物後、視野外的名條透名度
