...
function mod:UpdatePoisiton_Arrow(frame, shouldShow)
	if frame.TopArrow and (shouldShow ~= 2) and (self.db.targetGlow == "style3" or self.db.targetGlow == "style5" or self.db.targetGlow == "style6") then -- top arrow
		local topArrowSpace = -(self.db.targetGlow_y)
		...
		frame.TopArrow:Size(self.db.targetGlow_y_size)
	...
	if (frame.LeftArrow and frame.RightArrow) and (shouldShow ~= 2) and (self.db.targetGlow == "style4" or self.db.targetGlow == "style7" or self.db.targetGlow == "style8") then -- side arrows
		frame.RightArrow:Point("RIGHT", (frame.Portrait:IsShown() and frame.Portrait) or frame.HealthBar, "LEFT", self.db.targetGlow_x, 0)
		frame.LeftArrow:Point("LEFT", frame.HealthBar, "RIGHT", -(self.db.targetGlow_x), 0)
		frame.RightArrow:Size(self.db.targetGlow_x_size)
		frame.LeftArrow:Size(self.db.targetGlow_x_size)
...
function mod:UpdateElement_Glow(frame)
	...
	if UnitIsUnit(frame.unit, "target") and (self.db.targetGlow ~= "none") then
		...
		shouldShow = 1
		if self.db.lowHealthThreshold > 0 then
...
function mod:ConfigureElement_Glow(frame)
	...
	frame.TopArrow:SetTexture("Interface\\AddOns\\ElvUI\\media\\textures\\targetIndicator\\"..self.db.topArrowType)
	frame.LeftArrow:SetTexture("Interface\\AddOns\\ElvUI\\media\\textures\\nameplateTargetIndicatorLeft"..self.db.leftArrowType)
	frame.RightArrow:SetTexture("Interface\\AddOns\\ElvUI\\media\\textures\\nameplateTargetIndicatorRight"..self.db.leftArrowType)
end
...
function mod:ConstructElement_Glow(frame)
	...
	top:SetTexture("Interface\\AddOns\\ElvUI\\media\\textures\\targetIndicator\\"..self.db.topArrowType)
	top:Size(self.db.targetGlow_y_size)
	top:Hide()
	frame.TopArrow = top
	...
	left:Size(self.db.targetGlow_x_size)
	left:Hide()
	...
	right:Size(self.db.targetGlow_x_size)
	right:Hide()
