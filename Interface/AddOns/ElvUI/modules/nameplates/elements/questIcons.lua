...
function mod:GetQuests(unitID)
	...
	for i = 3, self.Tooltip:NumLines() do
		...
		if (not playerName or playerName == '' or playerName == E.myname) and progressText then
			...
			if QuestLogIndex then return QuestList end
		end
	end
...
function mod:Get_QuestIcon(frame, index)
	...
	skullIcon:SetTexture([[Interface\AddOns\ElvUI\media\textures\quest]])
	skullIcon:Hide()
...
function mod:QuestIcon_RelativePosition(frame, element)
	...
	if self.db.questIconPosition == "RIGHT" then
		if isCastbarLeft then
			frame.QuestIcon:SetPoint("LEFT", frame.CastBar.Icon, "RIGHT", self.db.questIconXOffset, self.db.questIconYOffset)
		elseif not isCastbarLeft and isEliteLeft then
			frame.QuestIcon:SetPoint("LEFT", frame.Elite, "RIGHT", self.db.questIconXOffset, self.db.questIconYOffset)
		else
			frame.QuestIcon:SetPoint("LEFT", frame.HealthBar, "RIGHT", self.db.questIconXOffset, self.db.questIconYOffset)
		end
	elseif self.db.questIconPosition == "LEFT" then
		if isCastbarRight then
			frame.QuestIcon:SetPoint("RIGHT", frame.CastBar.Icon, "LEFT", -(self.db.questIconXOffset), self.db.questIconYOffset)
		elseif not isCastbarRight and isEliteRight then
			frame.QuestIcon:SetPoint("RIGHT", frame.Elite, "LEFT", -(self.db.questIconXOffset), self.db.questIconYOffset)
		else
			frame.QuestIcon:SetPoint("RIGHT", frame.HealthBar, "LEFT", -(self.db.questIconXOffset), self.db.questIconYOffset)
		end
	end
end
...
function mod:ConstructElement_QuestIcons(frame)
	...
	questIcons:SetPoint("RIGHT", frame.HealthBar, "LEFT", -(self.db.questIconXOffset), self.db.questIconYOffset)
