				...
				if ( id ) then
					...
					specList[index + 1].func = function() 
						...
						for i=0, C_EquipmentSet.GetNumEquipmentSets()-1 do
							...
							if (spec ~= '') and (spec ~= NONE) and spec == name then
								specID = eid
								break;
							end
						end

						if (spec ~= '') and (spec ~= NONE) and specID then
							if C_EquipmentSet.GetEquipmentSetInfoByName(spec) then
								E:ScheduleTimer(C_EquipmentSet.UseEquipmentSet, 6, specID); 
							else
