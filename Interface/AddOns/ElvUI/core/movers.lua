...
local function CreateMover(parent, name, text, overlay, snapOffset, postdrag, shouldDisable, configString)
	...
	f.configString = configString or true
...
function E:CreateMover(parent, name, text, overlay, snapoffset, postdrag, moverTypes, shouldDisable, configString, var)
	...
	if E.CreatedMovers[name] == nil then
		...
		E.CreatedMovers[name].var = var or ""
	...
	for name in pairs(E.CreatedMovers) do
		if not show then
			...
			if E.CreatedMovers[name].type[moverType] then
				if E.CreatedMovers[name].var ~= '' and type(E.CreatedMovers[name].var) == 'string' and strfind(E.CreatedMovers[name].var,',') then
					local toggles = {split(',', E.CreatedMovers[name].var)}
					local toggle = E
					local i = 1
					while toggles[i] do
					...
					if E.db.general.hideDisableFrame and (not toggle) then
						_G[name]:Hide()
					else
						_G[name]:Show()
					end
