...
local function SetTemplate(f, t, glossTex, ignoreUpdates, forcePixelMode, isUnitFrameElement, trans)
	...
	if E.db.general.ShadowEnable and not E.db.general.transparent then f:CreateShadow(t) end
	...
	if t ~= 'NoBackdrop' then
		...
		if not E.private.general.pixelPerfect and not E.db.general.transparent and not f.forcePixelMode then
...
local function CreateShadow(f)
	...
	shadow:SetBackdrop({edgeFile = LSM:Fetch('border', 'ElvUI GlowBorder'), edgeSize = E:Scale(2)})
