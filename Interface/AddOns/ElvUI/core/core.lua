...
function E:IsPRType()
	...
	if ( instanceType == "party" or instanceType == "raid" or instanceType == "arena" or instanceType == "pvp" ) and InCombatLockdown() then
		return true
	else
		return false
	end
...
function E:SendMessage()
	...
	elseif IsInGroup() then
		...
		local EUIGVC = GetChannelName('EUIGVC')
		if EUIGVC and EUIGVC > 0 then
			C_ChatInfo_SendAddonMessage('EUI_VERSIONCHK', E.Version, 'CHANNEL', EUIGVC)
		elseif IsInGuild() then
			C_ChatInfo_SendAddonMessage('EUI_VERSIONCHK', E.Version, 'GUILD')
		end
	end
...
f:SetScript('OnUpdate', function(self, elapsed)
	...
	if self.delayed > 25 then
		...
		if numActiveChannels and (numActiveChannels >= 1) then
			if GetChannelName('EUIGVC') == 0 and numActiveChannels < MAX_WOW_CHAT_CHANNELS then
				JoinChannelByName('EUIGVC', nil, nil, true)
...
function E:UpdateDB()
	...
	E:GetModule('RAIDCD').db = E.db.euiscript
...
function E:UpdateMisc()
	...
	local RaidCD = E:GetModule('RAIDCD')
	RaidCD:CreateAnchor()
	RaidCD:ToggleRaidCD()
	
	E:GetModule('AutoButton'):ToggleAutoButton()
...
function E:Initialize(loginFrame)
	...
	if E.zhlocale then
		E:Print(L["捞月狗App 正式上线, 魔兽玩家聊天交友必备软件, 数据进度查询应有尽有.访问:http://www.gank.tv"])
