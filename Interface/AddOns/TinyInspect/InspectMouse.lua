...
local function AppendToGameTooltip(guid, ilevel, spec, weaponLevel, isArtifact)
    spec = spec or ""
    if(ElvUI and not ElvUI[1].db.tooltip.inspectInfo) then spec = "" end
    ...
    if (weaponLevel and weaponLevel > 0 and ElvUI[1].db.tooltip.inspectInfo) then
...
GameTooltip:HookScript("OnTooltipSetUnit", function(self)
	if(ElvUI and not ElvUI[1].db.tooltip.inspectInfo) then return end
