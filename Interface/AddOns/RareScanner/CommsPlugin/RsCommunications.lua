...
function RareScanner:ReportRareFound(npcID, vignetteInfo)
	...
	-- If its a NPC
	if (vignetteInfo.atlasName == RareScanner.NPC_VIGNETTE or vignetteInfo.atlasName == RareScanner.NPC_LEGION_VIGNETTE) then
	...
	if (not currentMap or currentMap == 0 or private.CONTINENT_ZONE_IDS[currentMap]) then
		return
	end
	...
	if (private.dbglobal.rares_found[npcID]) then
		...
		private.dbglobal.rares_found[npcID] = { mapID = currentMap, coordX = mapX, coordY = mapY, atlasName = atlas, foundTime = time() }
		RareScanner:PrintDebugMessage("DEBUG: Guardado en private.dbglobal.rares_found (ID: "..npcID.." MAPID: "..currentMap.." COORDX: "..mapX.." COORDY: "..mapY.." TIMESTAMP: "..time().." ATLASNAME: "..atlas)
	end

	-- Report to the GUILD, PARTY or RAID
	if (IsInGuild()) then
		local data = LibStub("AceSerializer-3.0"):Serialize({EVENT_NPC_FOUND, npcID, currentMap, mapX, mapY, atlas, time(), private.dbglobal.rares_loot[npcID] or {}, CHANNEL_GUILD, UnitName("player")})
		RareScanner:SendCommMessage(COMM_PREFIX, data, CHANNEL_GUILD)
	end
	if (IsInGroup(LE_PARTY_CATEGORY_HOME) or IsInRaid(LE_PARTY_CATEGORY_HOME)) then
		local data = LibStub("AceSerializer-3.0"):Serialize({EVENT_NPC_FOUND, npcID, currentMap, mapX, mapY, atlas, time(), private.dbglobal.rares_loot[npcID] or {}, IsInRaid(LE_PARTY_CATEGORY_HOME) and CHANNEL_RAID or CHANNEL_PARTY, UnitName("player")})
...
function RareScanner:DataReceived(_, data)
	...
	if (status) then
		...
		if (event == EVENT_NPC_FOUND) then
			...
			RareScanner:PrintDebugMessage("DEBUG: Rare recibido (ID: "..npcID.." MAPID: "..currentMap.." COORDX: "..x.." COORDY: "..y.." TIMESTAMP: "..timeStamp.." ATLASNAME: "..atlas.." CHANNEL: "..sourceChannel.." USER: "..requestOwner)
			...
			if (not private.dbglobal.rares_found[npcID]) then
				...
				elseif (RareScanner:ZoneFiltered(npcID)) then
					...
					private.dbglobal.rares_found[npcID] = { mapID = currentMap, coordX = x, coordY = y, atlasName = atlas, foundTime = timeStamp }
				end
			...
			if (sourceChannel == CHANNEL_GUILD) then
				if (IsInGroup(LE_PARTY_CATEGORY_HOME) or IsInRaid(LE_PARTY_CATEGORY_HOME)) then
					...
					if (not IsInInstance()) then
						...
						local resendData = LibStub("AceSerializer-3.0"):Serialize({EVENT_NPC_FOUND, npcID, currentMap, x, y, atlas, timeStamp, private.dbglobal.rares_loot[npcID] or {}, newChannel, UnitName("player")})
			...
			elseif (sourceChannel == CHANNEL_PARTY or sourceChannel == CHANNEL_RAID) then
				if (IsInGuild()) then
					...
					if (not RS_PlayerInGuild(requestOwner)) then
						...
						local resendData = LibStub("AceSerializer-3.0"):Serialize({EVENT_NPC_FOUND, npcID, currentMap, x, y, atlas, timeStamp, private.dbglobal.rares_loot[npcID] or {}, CHANNEL_GUILD, UnitName("player")})
			...
			if (responseData) then
				for k, v in pairs (responseData) do
					...
					if (not private.dbglobal.rares_found[k]) then
						private.dbglobal.rares_found[k] = { mapID = v.mapID, coordX = v.coordX, coordY = v.coordY, atlasName = v.atlasName, foundTime = v.foundTime }
						-- Add the loot to our loot list
						...
						if (IsInGuild() and SEND_DATA_GUILD_COOLDOWN and time() < SEND_DATA_GUILD_COOLDOWN + SEND_DATA_DELAY) then
							...
							if ((sourceChannel == CHANNEL_PARTY or sourceChannel == CHANNEL_RAID) and not RS_PlayerInGuild(requestOwner)) then
								RareScanner:PrintDebugMessage("DEBUG: Enviado informacion del raro recien descubierto a la hermandad")
								local resendData = LibStub("AceSerializer-3.0"):Serialize({EVENT_NPC_FOUND, k, v.mapID, v.coordX, v.coordY, v.atlasName, v.foundTime, v.lootTable, CHANNEL_GUILD, UnitName("player")})
								RareScanner:SendCommMessage(COMM_PREFIX, resendData, CHANNEL_GUILD)
	...
	if (next(private.dbglobal.rares_found) ~= nil) then
		...
		for k, v in pairs(private.dbglobal.rares_found) do
			...
			if (time() - v.foundTime <= _30_MINUTES) then
				...
				dataTable[k] = { mapID = v.mapID, coordX = v.coordX, coordY = v.coordY, atlasName = v.atlasName, foundTime = v.foundTime, lootTable = private.dbglobal.rares_loot[k] or {} }
			end
		end
