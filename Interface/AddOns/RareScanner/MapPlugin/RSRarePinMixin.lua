...
function RSRarePinMixin:OnAcquired(npcID, npcInfo)
	...
	self.isNpc = npcInfo.atlasName == RareScanner.NPC_VIGNETTE or npcInfo.atlasName == RareScanner.NPC_LEGION_VIGNETTE
	...
	if (atlasName == RareScanner.NPC_VIGNETTE) then
	...
	-- Eternal death
	if (self.isNpc and RS_tContains(private.PERMANENT_KILLS_ZONE_IDS, npcInfo.mapID)) then
		self.killable = true
	else
		self.killable = false
	end
...
function RSRarePinMixin:OnMouseDown(button)
	...
	if (button == "LeftButton" and IsShiftKeyDown()) then
		-- Ignored
		if (self.notDiscovered) then
			if (not private.dbchar.rares_not_discovered_ignored) then
				private.dbchar.rares_not_discovered_ignored = {}
			end
			private.dbchar.rares_not_discovered_ignored[self.npcID] = true
			private.RARES_NOT_DISCOVERED[self.mapID][self.npcID] = nil
			self:Hide();
		elseif (self.isNpc and self.killable) then
			self.killable = false
