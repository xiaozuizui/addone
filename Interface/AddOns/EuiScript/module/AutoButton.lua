...
local function GetWorldQuestItemList(toggle)
	...
	local taskInfo
	if mapID then
		taskInfo = C_TaskQuest_GetQuestsForPlayerByMapID(mapID)
...
function S:ScanItem(event)
	...
	elseif minimapZoneText == L["Salvage Yard"] then
		...
		if GetItemCount(123866) and (GetItemCount(123866) >= 5) and (not db.blankList[123866]) and (C_Map_GetBestMapForUnit("player") == 945) then
...
function S:Initialize()
	...
	-- Create anchor
	local AutoButtonAnchor = CreateFrame("Frame", "AutoButtonAnchor", E.UIParent)
	...
	E:CreateMover(AutoButtonAnchor, "AutoButtonAnchorMover", L["Auto QuestItem Button"], nil, nil, nil, "ALL,EUI", nil, "euiscript,autobutton", "db,euiscript,autobutton,enable")
	
	-- Create anchor2
	local AutoButtonAnchor2 = CreateFrame("Frame", "AutoButtonAnchor2", E.UIParent)
	...
	E:CreateMover(AutoButtonAnchor2, "AutoButtonAnchor2Mover", L["Auto InventoryItem Button"], nil, nil, nil, "ALL,EUI", nil, "euiscript,autobutton", "db,euiscript,autobutton,enable")
