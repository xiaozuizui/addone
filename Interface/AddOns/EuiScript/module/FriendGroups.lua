...
-- Author: Frunk
-- X-Curse-Packaged-Version: 7.14
-- modify for WOD 6.0 by eui.cc at 20141103
-- modify for WOD 6.2.4 by eui.cc at 20160328
-- modify for LEG 7.1.0 by eui.cc at 20161109
-- modify for LEG 7.1.5 by eui.cc at 20170214
-- modify for LEG 7.3 by eui.cc at 20170911
-- modify for LEG 8.0 by eui.cc at 20180721
...
local function FriendGroups_UpdateFriends()
	...
	for key,_ in pairs(E.db.euiscript.FriendGroups.collapsed) do
		if not GroupTotal[key] then
			E.db.euiscript.FriendGroups.collapsed[key] = nil
		end
	end
end
	...
	for i = 1, numBNetOffline do
		...
		for group in pairs(BnetFriendGroups[j]) do
            IncrementGroup(group)
			 if not E.db.euiscript.FriendGroups.collapsed[group] and not E.db.euiscript.FriendGroups.hide_offline then
                buttonCount = buttonCount + 1
...
local menu_items = {
	...
	[2] = {
		{ text = L["Hide all offline"], checked = function() return E.db.euiscript.FriendGroups.hide_offline end, func = function() CloseDropDownMenus() E.db.euiscript.FriendGroups.hide_offline = not E.db.euiscript.FriendGroups.hide_offline FriendGroups_Update() end },
		{ text = L["Colour names"], checked = function() return E.db.euiscript.FriendGroups.colour_classes end, func = function() CloseDropDownMenus() E.db.euiscript.FriendGroups.colour_classes = not E.db.euiscript.FriendGroups.colour_classes FriendGroups_Update() end },
	},
}
...
frame:SetScript("OnEvent", function(self, event, ...)
	...
	if event == "PLAYER_LOGIN" then
        ...
        if not E.db.euiscript.FriendGroups then
            E.db.euiscript.FriendGroups = {
                collapsed = {},
                hide_offline = false,
                colour_classes = true,
            }
