...
local buffSpell2 = GetSpellInfo(116670)--  镇魂�?
...
local function Update(self)
	...
	for i = 1, raidNum do
		...
		if subgroup <= floor(raidNum / 5) then
			local unit = "raid"..i
			local name,_,_,_,_,exipres,u = UnitAura(unit,buffName)
...
function WS:Initialize()
	...
	local f = CreateFrame("Button", "EuiMonkHeal", E.UIParent, "SecureActionButtonTemplate")
	...
	E:CreateMover(f, "EuiMonkHealMover", buffName, nil, nil, nil, "ALL,EUI", nil, "euiscript,wsbutton", "db,euiscript,wsbutton,enable")
