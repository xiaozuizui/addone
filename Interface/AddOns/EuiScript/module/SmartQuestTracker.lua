...
## Author: tyra314
## Version: 2.0.9
## Title: |cff1784d1ElvUI|r |cffFF6A00Smart Quest Tracker|r
...
P["euiscript"].SmartQuestTracker = false
...
function MyPlugin:InsertOptions()
	E.Options.args.ElvUI_SmartQuestTracker = {
		order = 100,
		type = "group",
		name = L["|cffFF6A00Smart Quest Tracker|r"],
		args = {
			enable = {
				order = 0,
				name = L['Enable'],
				type = 'toggle',
				get = function(info) return E.db.euiscript.SmartQuestTracker end,
				set = function(info, value) E.db.euiscript.SmartQuestTracker = value; MyPlugin:Toggle(); end,
			},
			clear = {
				order = 1,
				type = "group",
...
function MyPlugin:Toggle()
	if E.db.euiscript.SmartQuestTracker then
		...
		--Register event triggers
		MyPlugin:UnregisterEvent("ZONE_CHANGED")
		MyPlugin:UnregisterEvent("ZONE_CHANGED_NEW_AREA")
		MyPlugin:UnregisterEvent("QUEST_WATCH_UPDATE")
		MyPlugin:UnregisterEvent("QUEST_LOG_UPDATE")
		MyPlugin:UnregisterEvent("QUEST_ACCEPTED")
		MyPlugin:UnregisterEvent("WORLD_MAP_UPDATE")
...
E:RegisterModule(MyPlugin:GetName(), InitializeCallback)