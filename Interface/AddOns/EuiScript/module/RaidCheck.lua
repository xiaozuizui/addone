local E, _, DF = unpack(ElvUI) -- Import Functions/Constants, Config, Locales
...
elseif (GetLocale() == "zhTW") then
	...
	L.RuneNoBuff = "無強化符文："
	L.RuneAllBuff = "所有人都有強化符文."
else
	L.RaidCheckMsgOUTRAID = "you are not a team"
	L.RaidCheckMsgFullBuff = "EUI Buff check: conventional Buff complete!"
	L.RaidCheckMsgNoBuff = "EUI Buff check: the lack of the following"
	...
	L.BottomPanelRaidCheck = "EUI team checking tool"
	L.GameSetting = "open EUI set interface"
	...
	L.RuneNoBuff = "No rune: "
	L.RuneAllBuff = "Rune: all"	
end
...
--检查强化符文
function RC:CheckRune()
	local unitType=""
	if IsInRaid() then
		unitType="raid"
	elseif IsInGroup()  or IsPartyLFG() then
		unitType="party"
	else
		unitType=""
	end
	
	local norune = L.RuneNoBuff
	local Buffs = ""

	for r = 1, GetNumGroupMembers(), 1 do 
		local unit=unitType..r
		if UnitName(unit)==nil then
			unit="player"
		end

		local Charname = UnitName(unit)

		for i = 1, 40, 1 do
			local Buffname,_,_,_,_,_,_,_,_, SpellID = UnitAura(unit, i)  
			if SpellID then Buffs = Buffs..","..SpellID end
		end
		if string.find(Buffs, "175457") or --INT  
			string.find(Buffs, "175456") or --AGI 
			string.find(Buffs, "224001") or --new 7.2
			string.find(Buffs, "175439") then  --STR       
		else
			norune = norune..Charname..", "
		end
		Buffs = ""
	end
	if norune == L.RuneNoBuff then
		SendChatMessage(L.RuneAllBuff, E:CheckChat())
	else
		SendChatMessage(norune, E:CheckChat())
	end 
end
...
function RC:CheckRaidBuff()
	...
	if #NoBuff1 == 0 and #NoBuff2 == 0 and #NoBuff3 == 0 and #NoBuff4 == 0 and #NoBuff5 == 0 and #NoBuff6 == 0 and #NoBuff7 == 0 and #NoBuff8 == 0 and #NoBuff9 == 0 and #NoBuff10 == 0 then
		SendChatMessage(L.RaidCheckMsgFullBuff, E:CheckChat())
	else
		SendChatMessage(L.RaidCheckMsgNoBuff, E:CheckChat())
		...
		if #NoBuff1 > 0 then SendChatMessage(L.BuffType1.. ': '.. table.concat(NoBuff1, ","), E:CheckChat()) end
		if #NoBuff2 > 0 then SendChatMessage(L.BuffType2.. ': '.. table.concat(NoBuff2, ","), E:CheckChat()) end
		if #NoBuff3 > 0 then SendChatMessage(L.BuffType3.. ': '.. table.concat(NoBuff3, ","), E:CheckChat()) end
		if #NoBuff4 > 0 then SendChatMessage(L.BuffType4.. ': '.. table.concat(NoBuff4, ","), E:CheckChat()) end
		if #NoBuff5 > 0 then SendChatMessage(L.BuffType5.. ': '.. table.concat(NoBuff5, ","), E:CheckChat()) end
		if #NoBuff6 > 0 then SendChatMessage(L.BuffType6.. ': '.. table.concat(NoBuff6, ","), E:CheckChat()) end
		if #NoBuff7 > 0 then SendChatMessage(L.BuffType7.. ': '.. table.concat(NoBuff7, ","), E:CheckChat()) end
		if #NoBuff8 > 0 then SendChatMessage(L.BuffType8.. ': '.. table.concat(NoBuff8, ","), E:CheckChat()) end		
		if #NoBuff9 > 0 then SendChatMessage(L.BuffType9.. ': '.. table.concat(NoBuff9, ","), E:CheckChat()) end		
		if #NoBuff10 > 0 then SendChatMessage(L.RaidCheckBuff10.. ': '.. table.concat(NoBuff10, ","), E:CheckChat()) end
	end
...
function RC:CheckPosition()
			...
			if UnitIsConnected(unit) then
				if not UnitIsDeadOrGhost(unit) then
					...
					if DeadPlayerCount > 0 and DeadPlayerCount < 10 then
						DeadPlayer = DeadPlayer..(unitName or UNKNOWN).."."
					end
				...
				if OfflinePlayerCount > 0 and OfflinePlayerCount < 10 then
					OfflinePlayer = OfflinePlayer..(unitName or UNKNOWN).."."
				end
	...
	SendChatMessage(msg, E:CheckChat())
end
...
function RC:CheckRaidFlask()
	...
	for i = 1,raidNum do
		local subgroup
		if IsInRaid() then
			local _, _, sp = GetRaidRosterInfo(i)
			subgroup=sp
		end
		if (IsInRaid() and subgroup <= math.floor(raidNum/5)) or IsInGroup() or IsPartyLFG() then
			...
			local j = 1
			local has = 0
			local Bname, _ = UnitBuff(unit, j)
			while Bname and has == 0 and UnitIsConnected(unit) do
				if find(Bname, FlaskDATA) then
					has = 1
