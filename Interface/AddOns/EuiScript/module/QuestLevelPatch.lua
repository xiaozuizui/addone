...
function QuestLogQuests_hook(self, poiTable)
	...
	for button in QuestScrollFrame.titleFramePool:EnumerateActive() do
		local title, level, suggestedGroup, isHeader, isCollapsed, isComplete, frequency, questID, startEvent, displayQuestID, isOnMap, hasLocalPOI, isTask, isBounty, isStory, isHidden = GetQuestLogTitle(button.questLogIndex)
	...
	for i = 1, #elementsTable, 3 do
		if elementsTable[i] == QuestInfo_ShowTitle then
			if QuestInfoFrame.questLog then
				...
				local level = select(2, GetQuestLogTitle(questLogIndex)) or "?"
				local oldTitle = QuestInfoTitleHeader:GetText() or ""
				local newTitle = "["..level.."] "..oldTitle
