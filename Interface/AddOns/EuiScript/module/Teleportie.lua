...
local portSpellID = class == "MONK" and 119996 or 48020
local DB
...
local FindAura = AuraUtil.FindAuraByName -- is this really the way to do it?


local f = CreateFrame("FRAME", "EuiTeleportie", E.UIParent)
f:Hide()
f:SetSize(48, 48)
...
local function CheckPortalBuffFade()
	if not FindAura(BUFF_NAME, "player") then
		f:Hide()
		wipe(Portal)
...
local function CheckPortalBuffApply()
	if FindAura(BUFF_NAME, "player") then
...
--call with parameters to restore saved info or without to mark a new one
	...
	if savedInstId and (savedInstId ~= instance or not FindAura(BUFF_NAME, "player")) then wipe(Portal) return end
...
local function OnUpdate() 	
	...
	if x then
		...
		f.Direction:Show()
		f.Direction:SetRotation(atan2(y_diff, x_diff) - GetPlayerFacing())

		in_range = distance < 39.99
	else
		f.Direction:Hide()
		Distance_text:Hide()

		in_range = IsUsableSpell(portSpellID)
	end
