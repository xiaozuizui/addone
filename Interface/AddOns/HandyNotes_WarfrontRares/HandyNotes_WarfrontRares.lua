...
local npc_tooltip = CreateFrame("GameTooltip", "HandyNotesWR_npcToolTip", UIParent, "GameTooltipTemplate")
local tooltip_label

local function getCreatureNamebyID(id)
	npc_tooltip:SetOwner(UIParent, "ANCHOR_NONE")
	npc_tooltip:SetHyperlink(("unit:Creature-0-0-0-0-%d"):format(id))
	tooltip_label = _G["HandyNotesWR_npcToolTipTextLeft1"]:GetText()
end
...
function Arathi:OnInitialize()
    ...
    self.db = LibStub("AceDB-3.0"):New("HandyNotesWRDB", defaults, "Default");
...
function Arathi:RegisterWithHandyNotes()
    ...
    HandyNotes:RegisterPluginDB("HandyNotesWR", self, options)
...
function Arathi:Refresh()
    self:SendMessage("HandyNotes_NotifyUpdate", "HandyNotesWR")
end
